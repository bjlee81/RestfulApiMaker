...
// RowInterface defines the public API of the Cell.
type RowInterface interface {
	String() string
}

// String returns the value of a Row as a string.
...
// SheetInterface defines the public API of the Cell.
type SheetInterface interface {
	String() string
}

// String returns the value of a Sheet as a string.
...
func readRowsFromSheet(Worksheet *xlsxWorksheet, file *File) ([]*Row, int, int) {
	...
	for rowIndex := 0; rowIndex < rowCount; rowIndex++ {
		...
		for _, rawcell := range rawrow.C {
			...
			if x < len(row.Cells) {
				row.Value = strconv.Itoa(rowIndex)
...
func readSheetsFromZipFile(f *zip.File, file *File, sheetXMLMap map[string]string) ([]*Sheet, []string, error) {
	...
	for j := 0; j < sheetCount; j++ {
		...
		sheets[sheet.Index].Value = names[sheet.Index]
	}
